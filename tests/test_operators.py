...
def test_sigmoid(a: int) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert sigmoid(a - 2* eps) <= sigmoid(a - eps) <= sigmoid(a)


@pytest.mark.task0_2
@given(small_floats, small_ints, small_ints)
def test_transitive(a: float, delta1: float, delta2: float) -> None:
...
def test_other(a: float) -> None:
    ...
    """
    a = relu(a) + 1
    assert_close(log_back(a, 1), inv(a))
